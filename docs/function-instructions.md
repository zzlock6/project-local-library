...
find function - we want this function to return just ONE user from the given array which contains the criteria for parameter #2
...
sort function without creating new array just edit existing one & sorting them ascending order by Lname 
...
function??
we want to search all books 'borrows' array & return the total number of times an account has borrowed any book
sounds like we will use this to make a most active reader type of deal 
...
function??
we want take an account id & pull all the books they currently have checked out 
& push information from the author into the book object(s) returned 
...
find function we want the function to spit out ONE author that fits the critera in parameter 2 
...
find function we want the function to spit out ONE book that fits the critera in parameter 2 
