...
const { findAuthorById, 
        ...
        getBorrowersForBook  } = require("../public/src/books");

//function test find Author By ID 
describe("findAuthorById", () => {
  it("should return the correct author", () => {
    //contains 5 authors 0-4
    //parameter #2 calls for author 0
      const authors = [
        //correct result
        {
          id: 0,
   ...
   const expected = authors[0];
   const actual = findAuthorById(authors, 0);

      expect(actual).to.equal(expected);
  });
});

//function test find book by ID 
describe("findBookById", () => {
  it("should return the correct book", () => {
    //sample size of 3 books
    //parameter #2 calls for the 1st book in the sample data
      const books = [
        {
   ...
   const actual = findBookById(books, "5f447132d487bd81da01e25e");

      expect(actual).to.equal(expected);
  });
});

//funciton test partition books by borrowed status
describe("partitionBooksByBorrowedStatus", () => {
  it("should return all the books in the given array sorted into 2 arrays", () => {
    //sample size of 3 books
    //The first 2 books have NOT been returned, the last 1 has been returned
    //should return an array with 1 inLibrary 2 outLibrary
      const books = [
        //returned = false OutLibrary
        {
          id: "5f447132d487bd81da01e25e",
          title: "sit eiusmod occaecat eu magna",
          genre: "Science",
    ...
    const [outLibrary, inLibrary] = partitionBooksByBorrowedStatus(books)
    expect(outLibrary.length).to.equal(2);
    expect(inLibrary.length).to.equal(1);
  });
});

//function test get borrowers for book
describe("getBorrowersForBook", () => {
  it("should return the 10 most recent borrowers of the given book", () => {
    //sample data includes a book object with 25 total borrows & the entire accounts array that way 
    //the funciton can iterate through the accounts array skipping over those not in the result array 
    //tests include length of resulting array = 10 & that the first object in the result array
    //corresponds with the first borrower shown in borrows array 
    const book = {
      id: "5f447132d487bd81da01e25e",
      title: "sit eiusmod occaecat eu magna",
      genre: "Science",
    ...
    //most recent borrower is Kristen Woods ID 5f446f2e2cfa3e1d234679b9
    //line 835 for reference 
    const accounts = [
      {
   ...
   expect(actual.length).to.equal(10);
    const [first] = getBorrowersForBook(book, accounts);
   expect(first.age).to.equal(23)
   expect(first.name).to.eql({ first: "Kristen", last: "Woods" })
   expect(first.returned).to.be.false;
  });
});
